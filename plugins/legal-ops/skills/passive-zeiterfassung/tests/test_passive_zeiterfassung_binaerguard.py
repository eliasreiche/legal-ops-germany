"""Regressionstest: Binärdatei als --config darf keinen Traceback erzeugen,
...
from conftest import BEISPIEL_KONTEXT, lauf, schreibe  # noqa: E402
...
def test_cli_binaere_config_exit2_kein_traceback(tmp_path):
    termine = schreibe(tmp_path / "termine.json", {"termine": []})
    bin_datei = tmp_path / "config.json"
    bin_datei.write_bytes(_PNG_BYTES)

    ergebnis = lauf(EXECUTOR, "--termine", termine, "--kontext", BEISPIEL_KONTEXT,
                    "--config", bin_datei)
