"""Tests für plugins/legal-ops/skills/aktenkopf-extraktor/executor.py.
...
# mit dem gleichnamigen Executor anderer Skills (zitat-pruefer) im selben
# pytest-Prozess überschreibt.
_spec = importlib.util.spec_from_file_location("aktenkopf_extraktor_executor", EXECUTOR)
...
def test_cli_output_datei(tmp_path):
    ...
    assert report["meta"]["erzeugt_von"] == "aktenkopf-extraktor/executor.py"
...
def test_skill_md_trennt_datumsnennung_von_fristberechnung():
    ...
    assert "fristenrechner" in inhalt
