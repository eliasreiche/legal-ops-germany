...
> [CONVENTIONS.md](../../../../CONVENTIONS.md), Reifegrad-Leiter).
...
2. **Der Executor rechnet und klassifiziert** (kein eigenes Scoring durch das
   ...
   ```bash
   python3 plugins/compliance/skills/gwg-risiko-check/executor.py \
     --mandat <mandat.json> \
     [--output <report.json>]
   ```
...
4. **zitat-verifier-Lauf** als letzter Schritt: Die gerenderte Markdown-Doku
   wird durch [`zitat-verifier-de`](../../../../core/verify/zitat-verifier-de/SKILL.md)
   mit der mitgelieferten Registry geprüft, damit die §-Zitate nicht unmarkiert
   bleiben:

   ```bash
   python3 core/verify/zitat-verifier-de/executor.py \
     --input <akten-doku.md> \
     --registry plugins/compliance/skills/gwg-risiko-check/schema/quellen-registry.json
   ```
